...
function isLeapYear(date) {
  const leap = date.getFullYear();
  if (leap % 4 !== 0) return false;
  if (leap % 400 !== 0 && leap % 100 === 0) return false;
  return true;
}
...
function timeSpanToString(startDate, endDate) {
  ...
  const HH = Math.floor(gap / 1000 / 60 / 60)
    .toString()
    .padStart(2, 0);
  ...
  const ss = Math.floor((gap / 1000) % 60)
    .toString()
    .padStart(2, 0);
  const sss = (gap % 1000).toString().padStart(3, 0);
  return `${HH}:${mm}:${ss}.${sss}`;
}
